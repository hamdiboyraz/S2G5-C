...
function cumlelereDonustur(sentencesArray, sep = ",") {
...
function paragrafOlustur(sentencesArray, cb_cumlekur, cb_cumlelereDonustur) {
  const newArray = cb_cumlelereDonustur(sentencesArray, " ");

  const paragraph = cb_cumlekur(
    newArray[1],
    newArray[3],
    newArray[5],
    newArray[7],
  ...
  //     }
  //   });
  //   paragraph = cb_cumlekur(...paragraph);
...
function emojileriDonustur(message, emoji) {
  for (let key in emoji) {
    message = message.replaceAll(key, emoji[key]);
    message = message.replaceAll(key.toUpperCase(), emoji[key]);
  }
  return message;
}
